...
class Building:id:int;kind:int;x:int;y:int;demand:Counter[int]=field(default_factory=Counter);order:list[int]=field(default_factory=list)
@dataclass(slots=True)
class Pod:id:int;path:list[int]
@dataclass(slots=True)
class Candidate:
	score:int;cost:int;pad_id:int;module_id:int;astronaut_type:int;path:list[int]=field(default_factory=list);tubes:list[Pair]=field(default_factory=list);upgrades:list[Pair]=field(default_factory=list);teleport:Pair=None;delivered:int=0;services:list[tuple[int,int,int,int]]=field(default_factory=list);reroute_pod_id:int=None;destroy_pod_ids:list[int]=field(default_factory=list);extra_paths:list[list[int]]=field(default_factory=list)
	@property
	def efficiency(self):return self.score/max(1,self.cost)
class Planner:
	buildings:dict[int,Building];month:int;resources:int;score_so_far:int;tubes:dict[Pair,int];teleports:dict[int,int];pods:dict[int,Pod];exact_result:tuple
	def __init__(self):self.buildings={};self.month=0;self.resources=0;self.score_so_far=0;self.tubes={};self.teleports={};self.pods={};self.exact_result=None
	def play(self):
		while True:
			try:self.read_month()
			except EOFError:return
			print(";".join(self.choose_actions())or"WAIT");self.month+=1
	def read_month(self):
		self.resources=int(input());self.tubes={};self.teleports={};route_count=int(input())
		for _ in range(route_count):
			a,b,capacity=map(int,input().split())
			if capacity==0:self.teleports[a]=b
			else:self.tubes[route_key(a,b)]=capacity
		self.pods={};pod_count=int(input())
		for _ in range(pod_count):parts=list(map(int,input().split()));self.pods[parts[0]]=Pod(parts[0],parts[2:])
		new_buildings=[]
	...
	def choose_actions(self):
		...
		actions=[];serviced=self.get_serviced_pairs();service_counts=self.get_service_counts();module_load=self.get_module_load();degrees=self.get_tube_degrees();teleport_used=self.get_teleport_used_buildings();teleported_pairs=self.get_teleported_pairs();pod_ids=set(self.pods);planned_pods={pod_id:pod.path[:]for(pod_id,pod)in self.pods.items()};planned_teleports=dict(self.teleports);tubes=dict(self.tubes);direct_pod_counts=self.get_direct_service_pod_counts();edge_schedule=self.get_edge_schedule();dedicated_edge_counts=self.get_dedicated_pod_edge_counts();rerouted_pod_ids=set();retired_pod_ids=set();budget=self.resources;modules_by_type=self.get_modules_by_type();unserved_demands=self.get_unserved_demands(serviced);min_efficiency=self.min_efficiency();baseline_result=self.actual_score_from_pods(planned_pods,planned_teleports,tubes);baseline_score=baseline_result[0];print(self.score_debug_text("before",baseline_result),file=sys.stderr);exact_actions=[];exact_pods={pod_id:path[:]for(pod_id,path)in planned_pods.items()};exact_teleports=dict(planned_teleports);exact_tubes=dict(tubes);exact_budget=self.exact_plan(exact_actions,exact_pods,exact_teleports,exact_tubes,budget,set(pod_ids),baseline_score);exact_score=-1 if exact_budget is None else self.exact_result[0]
		demand=sum(sum(pad.demand.values())for pad in self.get_landing_pads())
		if exact_budget is not None and exact_score>baseline_score and self.exact_result[3]*10>=demand*9:
			after_score=self.score_debug_text("after",self.exact_result);self.score_so_far+=exact_score;print(f"resources_after {exact_budget} spent {self.resources-exact_budget} {after_score}",file=sys.stderr);return exact_actions
		for(pad,astronaut_type,count)in unserved_demands:
			if(pad.id,astronaut_type)in serviced:continue
			candidate=self.best_service_candidate(pad,astronaut_type,count,modules_by_type,module_load,degrees,teleport_used,tubes,edge_schedule,service_counts,planned_pods,planned_teleports,rerouted_pod_ids,budget,pod_ids)
			if candidate is not None and candidate.efficiency>=min_efficiency:
				budget=self.apply_candidate("service",candidate,actions,serviced,service_counts,module_load,degrees,teleport_used,planned_teleports,tubes,direct_pod_counts,edge_schedule,dedicated_edge_counts,planned_pods,budget,pod_ids)
				if candidate.reroute_pod_id is not None:rerouted_pod_ids.add(candidate.reroute_pod_id)
				if candidate.teleport is not None:teleported_pairs.add((candidate.pad_id,candidate.astronaut_type))
		while True:
			budget=self.destroy_obsolete_pods(actions,service_counts,edge_schedule,planned_pods,rerouted_pod_ids,retired_pod_ids,budget,pod_ids);speed_pick=self.best_speed_candidate(serviced,service_counts,tubes,direct_pod_counts,edge_schedule,dedicated_edge_counts,planned_pods,planned_teleports,rerouted_pod_ids,teleport_used,teleported_pairs,budget,pod_ids)
			if speed_pick is None:break
			reason,candidate=speed_pick
			if candidate.score<=0:break
			budget=self.apply_candidate(reason,candidate,actions,serviced,service_counts,module_load,degrees,teleport_used,planned_teleports,tubes,direct_pod_counts,edge_schedule,dedicated_edge_counts,planned_pods,budget,pod_ids)
			if candidate.reroute_pod_id is not None:rerouted_pod_ids.add(candidate.reroute_pod_id)
			if candidate.teleport is not None:teleported_pairs.add((candidate.pad_id,candidate.astronaut_type))
		chosen_score=self.actual_score_from_pods(planned_pods,planned_teleports,tubes)[0]
		if(exact_score,exact_budget or 0)>(chosen_score,budget):actions,planned_pods,planned_teleports=exact_actions,exact_pods,exact_teleports;tubes,budget,chosen_score=exact_tubes,exact_budget,exact_score
		if(chosen_score,budget)<=(baseline_score,self.resources):actions=[];planned_pods={pod_id:pod.path[:]for(pod_id,pod)in self.pods.items()};planned_teleports=dict(self.teleports);tubes=dict(self.tubes);budget=self.resources;chosen_score=baseline_score
		after_score=self.score_debug_text("after",planned_pods,planned_teleports,tubes);self.score_so_far+=chosen_score;print(f"resources_after {budget} spent {self.resources-budget} {after_score}",file=sys.stderr);return actions
	def exact_plan(self,actions,planned_pods,planned_teleports,tubes,budget,pod_ids,start_score=None):
		score=self.actual_score_from_pods;start_score=score(planned_pods,planned_teleports,tubes)[0]if start_score is None else start_score;degrees=Counter()
		for(a,b)in tubes:degrees[a]+=1;degrees[b]+=1
	...
	def exact_direct_teleports(self,actions,planned_pods,planned_teleports,tubes,teleport_used,budget):
		...
		while budget>=TELEPORT_COST:
			old_score=score(planned_pods,planned_teleports,tubes)[0];best=None
			for pad in self.get_landing_pads():
				if len(pad.demand)!=1 or pad.id in teleport_used:continue
				astronaut_type=next(iter(pad.demand))
				for module in modules_by_type[astronaut_type]:
					if module.id in teleport_used:continue
					new_teleports=dict(planned_teleports);new_teleports[pad.id]=module.id;gain=score(planned_pods,new_teleports,tubes)[0]-old_score
					if gain>0 and(best is None or(gain,-tube_cost(pad,module))>best[0]):best=(gain,-tube_cost(pad,module)),pad.id,module.id
			if best is None:break
			_,entrance_id,exit_id=best;actions.append(f"TELEPORT {entrance_id} {exit_id}");planned_teleports[entrance_id]=exit_id;teleport_used.add(entrance_id);teleport_used.add(exit_id);budget-=TELEPORT_COST
		return budget
	def best_exact_path_candidate(self,planned_pods,planned_teleports,tubes,degrees,edge_schedule,budget,pod_ids):
		...
		for path in route_options:
			...
			path_groups=[[path]]
			if len(path)==3 and path[0]==path[2]:path_groups+=[[path]*count for count in(3,EXACT_BULK_PODS)if len(pod_ids)+count<=MAX_PODS]
		...
		for(pod_id,old_path)in planned_pods.items():
			if pod_id not in self.pods or len(old_path)<=3:continue
			edges=[]
			for(a,b)in map(route_key,old_path,old_path[1:]):
				if(a,b)in tubes and(a,b)not in edges:edges.append((a,b))
			if not 2<=len(edges)<=4 or len(pod_ids)+len(edges)-1>MAX_PODS:continue
		...
		for(pod_id,old_path)in planned_pods.items():
			...
			if old_batches<=0:continue
			removed_schedule=self.schedule_without_pod(edge_schedule,self.pods[pod_id])
			for feeder in self.get_landing_pads():
				if feeder.id==old_pad.id:continue
				for(astronaut_type,count)in feeder.demand.items():
					for target in sorted(modules_by_type[astronaut_type],key=lambda item:tube_cost(hub,item))[:2]:
						if target.id==hub.id or len(pod_ids)>=MAX_PODS:continue
						rerouted_path=[old_pad.id]+[hub.id,old_pad.id]*(old_batches-1)+[hub.id];rerouted_path+=[target.id,hub.id]*((count+9)//10);paths=[rerouted_path,[feeder.id,hub.id,feeder.id]];new_tubes=[]
						for path in paths:new_tubes.extend(edge for edge in unique_new_tubes(path,tubes)if edge not in new_tubes)
						if not self.can_add_tubes(new_tubes,degrees,tubes):continue
		...
		if auto_rebalance_candidate is not None and(quick_best is None or(auto_rebalance_candidate.score,-auto_rebalance_candidate.cost)>(quick_best.score,-quick_best.cost)):
			quick_best=auto_rebalance_candidate
		...
		if auto_candidate is not None and(quick_best is None or(auto_candidate.score,-auto_candidate.cost)>(quick_best.score,-quick_best.cost)):quick_best=auto_candidate
		if quick_best is None or quick_best.score<1800:
			auto_tree_candidate=self.best_auto_tree_candidate(planned_pods,planned_teleports,tubes,degrees,edge_schedule,budget,pod_ids,old_score)
			if auto_tree_candidate is not None and(quick_best is None or(auto_tree_candidate.score,-auto_tree_candidate.cost)>(quick_best.score,-quick_best.cost)):quick_best=auto_tree_candidate
		if quick_best is not None:return quick_best
		...
		for(destroy_ids,base_paths,removed_schedule)in self.replacement_path_groups(planned_pods,tubes,edge_schedule):
			for path in bundle_options:
				...
				if len(pod_ids)-len(destroy_ids)+len(paths)>MAX_PODS:continue
				new_tubes=[]
				for added_path in paths:new_tubes.extend(edge for edge in unique_new_tubes(added_path,tubes)if edge not in new_tubes)
				if not self.can_add_tubes(new_tubes,degrees,tubes):continue
	...
	def bundle_upgrade_plan(self,paths,new_tubes,tubes,edge_schedule):
		...
		for(edge,required_capacity)in required_capacities.items():
			...
			for new_capacity in range(capacity+1,required_capacity+1):a,b=edge;cost+=tube_cost(self.buildings[a],self.buildings[b])*new_capacity;upgrades.append(edge)
		return cost,upgrades
	def exact_path_options(self,tubes):
		paths=[];seen=set();modules_by_type=self.get_modules_by_type()
		for pad in self.get_landing_pads():
			choices=[(astronaut_type,min(modules_by_type[astronaut_type],key=lambda item:tube_cost(pad,item)).id)for astronaut_type in sorted(pad.demand)];module_ids=[module_id for(_,module_id)in choices]
			if 1<len(module_ids)<=3:
				for ordered in permutations(module_ids):
					path=[pad.id]
					for module_id in ordered:path.extend([module_id,pad.id])
		...
		for pad in self.get_landing_pads():
			for astronaut_type in pad.demand:
				for module in sorted(modules_by_type[astronaut_type],key=lambda item:tube_cost(pad,item))[:2]:
					...
					if tuple(path)not in seen:paths.append(path);seen.add(tuple(path))
					for via in sorted((building for building in self.buildings.values()if building.id not in(pad.id,module.id)),key=lambda item:tube_cost(pad,item)+tube_cost(item,module))[:2]:
						for path in two_hop_loop_paths(pad.id,via.id,module.id):
							if tuple(path)not in seen:paths.append(path);seen.add(tuple(path))
		building_ids=sorted(self.buildings);cheap_pairs=sorted((tube_cost(self.buildings[a],self.buildings[b]),route_key(a,b))for(index,a)in enumerate(building_ids)for b in building_ids[index+1:]if route_key(a,b)not in tubes)[:24]
		for(a,b)in list(tubes)+[pair for(_,pair)in cheap_pairs]:
			for path in([a,b,a],[b,a,b]):
				if tuple(path)not in seen:paths.append(path);seen.add(tuple(path))
		return paths[:EXACT_ROUTE_LIMIT]
	def star_connect_options(self,tubes):
		paths=[];seen=set()
		for pad in self.get_landing_pads():
			nearby=sorted((tube_cost(pad,building),building.id)for building in self.buildings.values()if building.id!=pad.id)[:5]
			for(index,(_,a))in enumerate(nearby):
				for(_,b)in nearby[index+1:]:
					full_tubes=dict(tubes);full_tubes[route_key(pad.id,a)]=1;full_tubes[route_key(pad.id,b)]=1
		...
		for count in range(1,len(pods)+1):
			for removed in combinations(pods,count):
				...
				for pod in removed:
					for(a,b)in map(route_key,planned_pods[pod.id],planned_pods[pod.id][1:]):
						if(a,b)in tubes and(a,b)not in edges:edges.append((a,b))
				if not 1<=len(edges)<=6:continue
				schedule=edge_schedule.copy()
				for pod in removed:schedule=self.schedule_without_pod(schedule,pod)
				paths=[]
				for route in self.greedy_edge_routes(edges,tubes):
					paths.append(route)
					for(a,b)in edges:
						if route[-1]==a:paths+=[route+[b,a]*repeat+[b]for repeat in range(1,4)]
						elif route[-1]==b:paths+=[route+[a,b]*repeat+[a]for repeat in range(1,4)]
				seen=set()
				for path in paths:
					key=tuple(path)
					if key not in seen:
	...
	def resolve_auto_routes(self,specs:list[tuple[int,list[Pair]]])->dict[int,list[int]]:
		...
		def service_loads(graph):
			...
			for(node,waiting)in queues.items():
				...
				kind=self.buildings[node].kind
				for passenger in waiting:
					astronaut_type=passenger[2]
					if kind==astronaut_type:continue
		...
		def capacity_moves(requests):
			...
			for(edge,pods)in by_tube.items():
				for(pod_id,move)in sorted(pods)[:self.tubes.get(edge,0)]:moves[pod_id]=move
			return moves
		...
		for astronaut_type in{item for building in self.get_landing_pads()for item in building.demand}:
			...
			while queue:
				current=queue.popleft()
				for(neighbor,cost)in reverse_graph.get(current,[]):
					if distances[astronaut_type][current]+cost<distances[astronaut_type][neighbor]:
						distances[astronaut_type][neighbor]=distances[astronaut_type][current]+cost
		...
		auto_ids=set(graphs);module_arrivals=Counter();service_delivered=Counter();service_speed=Counter();service_balance=Counter();fixed_pods={pid:pod.path[:]for(pid,pod)in self.pods.items()if pid not in auto_ids};pod_positions={pid:0 for pid in fixed_pods};base_counts=Counter();pod_service_counts={pid:Counter()for pid in graphs}
		for path in fixed_pods.values():
	...
	def greedy_edge_routes(self,edges,tubes):
		...
		def loads(queues):
			result=Counter()
			for(node,waiting)in queues.items():
				for astronaut_type in waiting:
					options=[neighbor for neighbor in graph.get(node,[])if distances[astronaut_type][neighbor]<distances[astronaut_type][node]]
					if options:result[node,min(options,key=lambda item:(distances[astronaut_type][item],item))]+=1
			return result
		def run(start,prefer_current):
			queues={node:[astronaut_type for astronaut_type in self.buildings[node].order if distances[astronaut_type][node]<far]if self.buildings[node].kind==0 else[]for node in graph};current=start;path=[current]
			for _ in range(MONTH_DAYS):
				kind=self.buildings[current].kind
				if kind>0:queues[current]=[astronaut_type for astronaut_type in queues[current]if astronaut_type!=kind]
				waiting_edges=loads(queues)
				if not waiting_edges:break
				source_load=Counter()
				for((source,_),count)in waiting_edges.items():source_load[source]=max(source_load[source],count)
				if prefer_current and source_load[current]:source=current
				else:big=[source for(source,count)in source_load.items()if count>=10];source=min(big,key=lambda item:(0 if item==current else 1,item))if big else max(source_load,key=lambda item:(source_load[item],-item))
				if source!=current:current=next_step(current,source);path.append(current);continue
				target=max((b for(a,b)in waiting_edges if a==current),key=lambda item:(waiting_edges[current,item],-item));moved=[];kept=[];seats=10
				for astronaut_type in queues[current]:
					if seats and distances[astronaut_type][target]<distances[astronaut_type][current]:moved.append(astronaut_type);seats-=1
					else:kept.append(astronaut_type)
				queues[current]=kept;queues[target].extend(moved);current=target;path.append(current)
			return path
		graph={}
		for(a,b)in edges:graph.setdefault(a,[]).append(b);graph.setdefault(b,[]).append(a)
		...
		if len(seen_nodes)<len(graph):return[]
		full_graph={}
		for(a,b)in tubes:full_graph.setdefault(a,[]).append(b);full_graph.setdefault(b,[]).append(a)
		far=10**9;distances={}
		for astronaut_type in{item for building in self.get_landing_pads()for item in building.demand}:
			distances[astronaut_type]={building_id:far for building_id in self.buildings};queue=deque(building.id for building in self.buildings.values()if building.kind==astronaut_type)
			for building_id in queue:distances[astronaut_type][building_id]=0
		...
		initial=loads({node:[astronaut_type for astronaut_type in self.buildings[node].order if distances[astronaut_type][node]<far]if self.buildings[node].kind==0 else[]for node in graph});starts=sorted({source for(source,_)in initial},key=lambda item:(-sum(count for((source,_),count)in initial.items()if source==item),item));result=[];seen=set()
		for start in starts[:4]or sorted(graph)[:1]:
			for prefer_current in(True,False):
				path=run(start,prefer_current)
				if len(path)>1 and tuple(path)not in seen:result.append(path);seen.add(tuple(path))
		return result[:8]
	def apply_exact_path(self,candidate,actions,planned_pods,tubes,degrees,edge_schedule,pod_ids,budget):
		destroy_ids=candidate.destroy_pod_ids or([]if candidate.reroute_pod_id is None else[candidate.reroute_pod_id])
		for pod_id in destroy_ids:
			actions.append(f"DESTROY {pod_id}");pod_ids.discard(pod_id)
			for(edge,day)in path_edge_days(planned_pods.pop(pod_id)):
	...
	def destroy_obsolete_pods(self,actions,service_counts,edge_schedule,planned_pods,rerouted_pod_ids,retired_pod_ids,budget,pod_ids):
		while(obsolete_pod:=self.obsolete_pod(service_counts,rerouted_pod_ids|retired_pod_ids)):
			actions.append(f"DESTROY {obsolete_pod.id}");retired_pod_ids.add(obsolete_pod.id);pod_ids.discard(obsolete_pod.id);del planned_pods[obsolete_pod.id]
			for(edge,day)in path_edge_days(obsolete_pod.path):
				edge_schedule[edge,day]-=1
				if edge_schedule[edge,day]<=0:del edge_schedule[edge,day]
			services=self.pod_services(obsolete_pod)
			for(pad_id,astronaut_type,_,_)in services:service_counts[pad_id,astronaut_type]-=1
			budget+=POD_REFUND
		return budget
	...
	def score_debug_text(self,label,planned_pods,planned_teleports=None,planned_tubes=None):
		if isinstance(planned_pods,tuple):score,speed,balance,delivered,_,_=planned_pods
		else:score,speed,balance,delivered,_,_=self.actual_score_from_pods(planned_pods,planned_teleports,planned_tubes)
		demand=sum(sum(pad.demand.values())for pad in self.get_landing_pads());return f"score_{label} month {score} total_so_far {self.score_so_far+score} speed {speed} diversity {balance} delivered {delivered}/{demand} stranded {demand-delivered}"
	def actual_score_from_pods(self,planned_pods,planned_teleports=None,planned_tubes=None):
		...
		if planned_tubes is None:
			for path in planned_pods.values():
				for(a,b)in zip(path,path[1:]):tubes.setdefault(route_key(a,b),1)
		reverse_edges={}
		for(a,b)in tubes:reverse_edges.setdefault(a,[]).append((b,1));reverse_edges.setdefault(b,[]).append((a,1))
		for(a,b)in teleports.items():reverse_edges.setdefault(b,[]).append((a,0))
		distances={}
		for astronaut_type in{astronaut_type for pad in self.get_landing_pads()for astronaut_type in pad.demand}:
			distances[astronaut_type]={building_id:10**9 for building_id in self.buildings};queue=deque()
			for building in self.buildings.values():
				if building.kind==astronaut_type:distances[astronaut_type][building.id]=0;queue.append(building.id)
			while queue:
				...
				for(neighbor_id,cost)in reverse_edges.get(building_id,[]):
					...
					if distance<distances[astronaut_type][neighbor_id]:
						...
						if cost==0:queue.appendleft(neighbor_id)
						else:queue.append(neighbor_id)
		queues={}
		for pad in self.get_landing_pads():queues[pad.id]=[(pad.id,index,astronaut_type)for(index,astronaut_type)in enumerate(pad.order)]
		module_arrivals=Counter();service_delivered=Counter();service_speed=Counter();service_balance=Counter();pod_positions={pod_id:0 for pod_id in planned_pods};capacity_tubes=planned_tubes if planned_tubes is not None else None
		for day in range(MONTH_DAYS):self.apply_teleport_phase(queues,distances,teleports);self.settle_node_arrivals(day,queues,module_arrivals,service_delivered,service_speed,service_balance);self.launch_pods(queues,self.daily_pod_moves(planned_pods,pod_positions,capacity_tubes),distances,pod_positions,planned_pods);self.settle_node_arrivals(day+1,queues,module_arrivals,service_delivered,service_speed,service_balance)
		speed=sum(service_speed.values());balance=sum(service_balance.values());delivered=sum(service_delivered.values());return speed+balance,speed,balance,delivered,service_delivered,{}
	def apply_teleport_phase(self,queues,distances,teleports):
		for(entrance_id,exit_id)in sorted(teleports.items()):
			waiting=[]
			for passenger in queues.get(entrance_id,[]):
				astronaut_type=passenger[2]
				if distances[astronaut_type][exit_id]<=distances[astronaut_type][entrance_id]:queues.setdefault(exit_id,[]).append(passenger)
				else:waiting.append(passenger)
			if waiting:queues[entrance_id]=waiting
			elif entrance_id in queues:del queues[entrance_id]
	def settle_node_arrivals(self,day,queues,module_arrivals,service_delivered,service_speed,service_balance):
		for building_id in sorted(queues):
			building=self.buildings[building_id]
			if building.kind<=0:continue
			waiting=[]
			for passenger in queues[building_id]:
				pad_id,_,astronaut_type=passenger
				if building.kind!=astronaut_type:waiting.append(passenger);continue
				group=pad_id,astronaut_type;service_speed[group]+=max(0,50-day);service_balance[group]+=max(0,50-module_arrivals[building_id]);service_delivered[group]+=1;module_arrivals[building_id]+=1
			if waiting:queues[building_id]=waiting
			else:del queues[building_id]
	def daily_pod_moves(self,planned_pods,pod_positions,capacity_tubes):
		requests={}
	...
	def score_from_pods(self,planned_pods,planned_teleports=None):
		...
		module_arrivals=Counter();service_delivered=Counter();service_speed=Counter();service_balance=Counter()
		for day in range(MONTH_DAYS):
			for(pair,path)in service_paths.items():self.apply_instant_edges(path,queues[pair],pod_edges,teleport_edges)
			self.settle_score_arrivals(day,service_paths,queues,module_arrivals,service_delivered,service_speed,service_balance);moved={pair:[0]*len(path)for(pair,path)in service_paths.items()};edge_waiters={}
			for(pair,path)in service_paths.items():
				for(index,waiting)in enumerate(queues[pair][:-1]):
					edge=path[index],path[index+1]
					if waiting and edge in pod_edges:edge_waiters.setdefault(edge,[]).append((pair,index))
			for(edge,waiters)in edge_waiters.items():
				capacity=10*directed_schedule[edge,day]
				for(pair,index)in sorted(waiters):
					boarded=min(queues[pair][index],capacity);queues[pair][index]-=boarded;moved[pair][index+1]+=boarded;capacity-=boarded
					if capacity<=0:break
			for(pair,path)in service_paths.items():
				for(index,count)in enumerate(moved[pair]):queues[pair][index]+=count
			self.settle_score_arrivals(day+1,service_paths,queues,module_arrivals,service_delivered,service_speed,service_balance)
		speed=sum(service_speed.values());balance=sum(service_balance.values());delivered=sum(service_delivered.values());return speed+balance,speed,balance,delivered,service_delivered,service_paths
	def apply_instant_edges(self,path,queues,pod_edges,teleport_edges):
		changed=True
		while changed:
			changed=False
			for(index,waiting)in enumerate(queues[:-1]):
				edge=path[index],path[index+1]
				if waiting and(edge in teleport_edges or edge not in pod_edges):queues[index+1]+=waiting;queues[index]=0;changed=True
	def settle_score_arrivals(self,day,service_paths,queues,module_arrivals,service_delivered,service_speed,service_balance):
		for(pair,path)in service_paths.items():
			arrived=queues[pair][-1]
			if arrived<=0:continue
			module_id=path[-1];queues[pair][-1]=0
			for _ in range(arrived):speed_points=max(0,50-day);balance_points=max(0,50-module_arrivals[module_id]);service_speed[pair]+=speed_points;service_balance[pair]+=balance_points;service_delivered[pair]+=1;module_arrivals[module_id]+=1
	def get_serviced_pairs(self):return set(self.get_service_counts())
	def get_service_counts(self):
		counts=Counter()
		for(entrance,exit_id)in self.teleports.items():
			if entrance in self.buildings and exit_id in self.buildings and self.buildings[entrance].kind==0 and self.buildings[exit_id].kind>0:counts[entrance,self.buildings[exit_id].kind]+=1
		for pod in self.pods.values():
			for(pad_id,astronaut_type,_,_)in self.pod_services(pod):counts[pad_id,astronaut_type]+=1
		for pair in self.get_reachable_services():counts[pair]=max(1,counts[pair])
		return counts
	def pod_services(self,pod):
		if not pod.path or pod.path[0]not in self.buildings or self.buildings[pod.path[0]].kind!=0:return[]
		services=[];seen_types=set()
		for(distance,building_id)in enumerate(pod.path[1:],1):
			if building_id not in self.buildings or self.buildings[building_id].kind<=0 or self.buildings[building_id].kind in seen_types:continue
			seen_types.add(self.buildings[building_id].kind);services.append((pod.path[0],self.buildings[building_id].kind,building_id,distance))
		return services
	def get_reachable_services(self,skip_pod_id=None):
		adjacency=self.get_pod_adjacency(skip_pod_id);services={}
		for pad in self.get_landing_pads():
			queue=deque([pad.id]);distances={pad.id:0}
			while queue:
				building_id=queue.popleft();building=self.buildings[building_id]
				if building.kind>0 and building.kind in pad.demand and(pad.id,building.kind)not in services:services[pad.id,building.kind]=building_id,distances[building_id]
				for neighbor_id in adjacency.get(building_id,[]):
					if neighbor_id in distances:continue
					distances[neighbor_id]=distances[building_id]+1;queue.append(neighbor_id)
		return services
	def get_pod_adjacency(self,skip_pod_id=None):return self.adjacency_from_paths([pod.path for pod in self.pods.values()if pod.id!=skip_pod_id])
	def adjacency_from_paths(self,paths,teleports=None):
		adjacency={}
		for path in paths:
			for(a,b)in zip(path,path[1:]):adjacency.setdefault(a,[]).append(b)
		for(a,b)in(self.teleports if teleports is None else teleports).items():adjacency.setdefault(a,[]).append(b)
		return adjacency
	def get_module_load(self):
		loads=Counter()
		for((pad_id,astronaut_type),(module_id,_))in self.get_reachable_services().items():loads[module_id]+=self.buildings[pad_id].demand[astronaut_type]
		return loads
	def get_tube_degrees(self):
		degrees=Counter()
		for(a,b)in self.tubes:degrees[a]+=1;degrees[b]+=1
		return degrees
	def get_teleport_used_buildings(self):used=set(self.teleports);used.update(self.teleports.values());return used
	def get_teleported_pairs(self):
		pairs=set()
		for(entrance,exit_id)in self.teleports.items():
			if entrance in self.buildings and exit_id in self.buildings and self.buildings[entrance].kind==0 and self.buildings[exit_id].kind>0:pairs.add((entrance,self.buildings[exit_id].kind))
		return pairs
	...
	def get_edge_schedule(self):
		schedule=Counter()
		for pod in self.pods.values():
			for(edge,day)in path_edge_days(pod.path):schedule[edge,day]+=1
		return schedule
	def directed_schedule_from_paths(self,paths):
		schedule=Counter()
		for path in paths:
			for(edge,day)in directed_path_edge_days(path):schedule[edge,day]+=1
		return schedule
	def get_unserved_demands(self,serviced):
		demands=[]
		for pad in self.get_landing_pads():
			for(astronaut_type,count)in pad.demand.items():
				if(pad.id,astronaut_type)not in serviced:demands.append((pad,astronaut_type,count))
		return sorted(demands,key=lambda demand:(-demand[2],demand[0].id,demand[1]))[:260]
	def best_service_candidate(self,pad,astronaut_type,count,modules_by_type,module_load,degrees,teleport_used,tubes,edge_schedule,service_counts,planned_pods,planned_teleports,rerouted_pod_ids,budget,pod_ids):
		best=None;best_key=None;planned_adjacency=self.adjacency_from_paths(list(planned_pods.values()),planned_teleports);targets=[(module,module_load[module.id])for module in self.best_modules(modules_by_type[astronaut_type],pad,module_load)]
		for(entrance_id,exit_id)in planned_teleports.items():
			if self.buildings[exit_id].kind==astronaut_type and entrance_id!=pad.id:targets.append((self.buildings[entrance_id],module_load[exit_id]))
		for(module,current_load)in targets:
			candidates=self.service_candidates(pad,module,astronaut_type,count,current_load,module_load,degrees,teleport_used,tubes,edge_schedule,service_counts,planned_pods,planned_teleports,planned_adjacency,rerouted_pod_ids,budget,pod_ids)
			for candidate in candidates:
				candidate_key=candidate.score,candidate.delivered,len(candidate.services)or 1,candidate.efficiency
				if best is None or candidate_key>best_key:best=candidate;best_key=candidate_key
		for candidate in self.multi_service_candidates(pad,astronaut_type,modules_by_type,module_load,degrees,tubes,edge_schedule,service_counts,planned_pods,planned_teleports,budget,pod_ids):
			candidate_key=candidate.score,candidate.delivered,len(candidate.services)or 1,candidate.efficiency
			if best is None or candidate_key>best_key:best=candidate;best_key=candidate_key
		for candidate in self.transfer_service_candidates(pad,astronaut_type,degrees,tubes,edge_schedule,service_counts,planned_adjacency,planned_pods,planned_teleports,budget,pod_ids):
			candidate_key=candidate.score,candidate.delivered,len(candidate.services)or 1,candidate.efficiency
			if best is None or candidate_key>best_key:best=candidate;best_key=candidate_key
		return best
	def get_modules_by_type(self):
		modules_by_type={}
		for building in self.buildings.values():
			if building.kind>0:modules_by_type.setdefault(building.kind,[]).append(building)
		return modules_by_type
	def get_landing_pads(self):return[building for building in self.buildings.values()if building.kind==0 and building.demand]
	def best_modules(self,modules,pad,module_load):return sorted(modules,key=lambda module:(module_load[module.id]//20,tube_cost(pad,module)))[:4]
	def multi_service_candidates(self,pad,required_type,modules_by_type,module_load,degrees,tubes,edge_schedule,service_counts,planned_pods,planned_teleports,budget,pod_ids):
		if len(pod_ids)>=MAX_PODS:return[]
		demands=[]
		for(astronaut_type,count)in pad.demand.items():
			if(pad.id,astronaut_type)not in service_counts and astronaut_type in modules_by_type:module=self.best_modules(modules_by_type[astronaut_type],pad,module_load)[0];demands.append((astronaut_type,count,module))
		if len(demands)<2 or all(astronaut_type!=required_type for(astronaut_type,_,_)in demands):return[]
		candidates=[];old_score=self.score_from_pods(planned_pods,planned_teleports)[0]
		for size in range(2,min(4,len(demands))+1):
			for ordered in permutations(demands,size):
				if all(astronaut_type!=required_type for(astronaut_type,_,_)in ordered):continue
				star_path=[pad.id]
				for(_,_,module)in ordered:star_path.extend([module.id,pad.id])
				...
				for(path,first_days)in((star_path,[2*index-1 for index in range(1,size+1)]),(chain_path,list(range(1,size+1)))):
					...
					if score<=0:continue
					delivered_total=0;services=[];period=len(path)-1
					for(first_day,(astronaut_type,count,module))in zip(first_days,ordered):delivered=monthly_pod_deliveries(count,first_day,1,period);delivered_total+=delivered;services.append((pad.id,astronaut_type,module.id,delivered))
					first_type,_,first_module=ordered[0];candidate=Candidate(score,cost,pad.id,first_module.id,first_type,path,new_tubes,upgrades,delivered=delivered_total,services=services);candidates.append(candidate)
		return candidates
	def transfer_service_candidates(self,pad,required_type,degrees,tubes,edge_schedule,service_counts,planned_adjacency,planned_pods,planned_teleports,budget,pod_ids):
		if len(pod_ids)>=MAX_PODS:return[]
		entries=[]
		for building in sorted(self.buildings.values(),key=lambda item:tube_cost(pad,item)):
			if building.id==pad.id or route_key(pad.id,building.id)in tubes:continue
			services=self.reachable_service_entries(pad,building.id,service_counts,planned_adjacency)
			if services:entries.append((building,services))
		candidates=[];old_score=self.score_from_pods(planned_pods,planned_teleports)[0];by_type=self.get_modules_by_type()
		for(entry,entry_services)in entries[:8]:
			...
			for missing in pad.demand:
				if(pad.id,missing)in service_counts or missing in served_types or missing not in by_type:continue
				if required_type not in served_types and required_type!=missing:continue
		...
		for size in range(1,min(3,len(entries))+1):
			for ordered in permutations(entries[:8],size):
				if not any(required_type in[astronaut_type for(astronaut_type,_,_)in services]for(_,services)in ordered):continue
				path=[pad.id]
				for(entry,_)in ordered:path.extend([entry.id,pad.id])
				paths=[path]
				if len(ordered)>1:
					...
					for(entry,entry_services)in ordered:
						...
						for _ in range(repeats):repeated_path.extend([entry.id,pad.id])
				...
				for path in paths:
					...
					score=self.score_added_path(planned_pods,planned_teleports,old_score,path)
					if score<=0:continue
					delivered_total=0;services=[];seen_types=set()
					for(entry,entry_services)in ordered:
						entry_day=path.index(entry.id)
						for(astronaut_type,module_id,distance)in entry_services:
							if astronaut_type in seen_types:continue
							seen_types.add(astronaut_type);delivered=monthly_pod_deliveries(pad.demand[astronaut_type],entry_day+distance,1,len(path)-1);delivered_total+=delivered;services.append((pad.id,astronaut_type,module_id,delivered))
					if not services or all(astronaut_type!=required_type for(_,astronaut_type,_,_)in services):continue
					_,first_type,first_module,_=services[0];candidates.append(Candidate(score,cost,pad.id,first_module,first_type,path,new_tubes,upgrades,delivered=delivered_total,services=services))
		return candidates
	def service_candidates(self,pad,module,astronaut_type,count,current_load,module_load,degrees,teleport_used,tubes,edge_schedule,service_counts,planned_pods,planned_teleports,planned_adjacency,rerouted_pod_ids,budget,pod_ids):
		candidates=[];route_options=[];has_tube_candidate=False;existing_path=self.shortest_tube_path(pad.id,module.id,tubes)
		if existing_path is not None and len(existing_path)<=7:
			candidate_path=loop_path(existing_path);route_options.append((candidate_path,[],len(existing_path)-1,2*(len(existing_path)-1)))
			if len(pod_ids)<MAX_PODS:
				upgrade_cost,upgrades=self.path_upgrade_plan(candidate_path,[],tubes,edge_schedule);candidate_cost=POD_COST+upgrade_cost
				if candidate_cost<=budget:score,delivered,services=self.entry_service_bundle(pad,module.id,len(existing_path)-1,2*(len(existing_path)-1),module_load,service_counts,planned_adjacency);candidate=Candidate(score,candidate_cost,pad.id,module.id,astronaut_type,candidate_path,upgrades=upgrades,delivered=delivered,services=services);candidates.append(candidate);has_tube_candidate=True
			if len(existing_path)>2 and self.path_has_pod_coverage(existing_path[:-1],planned_pods):
				transfer_path=[existing_path[-2],module.id,existing_path[-2]];first_day=2*(len(existing_path)-2)+1;route_options.append((transfer_path,[],first_day,2))
				if len(pod_ids)<MAX_PODS:
					upgrade_cost,upgrades=self.path_upgrade_plan(transfer_path,[],tubes,edge_schedule);transfer_cost=POD_COST+upgrade_cost
					if transfer_cost<=budget:delivered=monthly_pod_deliveries(count,first_day,1,2);score=monthly_score(delivered,first_day,current_load,period=2)*self.months_left();candidate=Candidate(score,transfer_cost,pad.id,module.id,astronaut_type,transfer_path,upgrades=upgrades,delivered=delivered);candidates.append(candidate);has_tube_candidate=True
		direct_tubes=[]if route_key(pad.id,module.id)in tubes else[(pad.id,module.id)];path=[pad.id,module.id,pad.id]
		if self.can_add_tubes(direct_tubes,degrees,tubes):
			route_options.append((path,direct_tubes,1,2))
			if len(pod_ids)<MAX_PODS:
				upgrade_cost,upgrades=self.path_upgrade_plan(path,direct_tubes,tubes,edge_schedule);direct_cost=sum(tube_cost(self.buildings[a],self.buildings[b])for(a,b)in direct_tubes)+POD_COST+upgrade_cost
				if direct_cost<=budget:score,delivered,services=self.entry_service_bundle(pad,module.id,1,2,module_load,service_counts,planned_adjacency);candidates.append(Candidate(score,direct_cost,pad.id,module.id,astronaut_type,path,direct_tubes,upgrades,delivered=delivered,services=services));has_tube_candidate=True
		if not has_tube_candidate and count>=8:
			two_hop_options=0
			for via in self.two_hop_buildings(pad,module):
				if via.id in(pad.id,module.id):continue
				path=[pad.id,via.id,module.id];two_hop_tubes=[edge for edge in zip(path,path[1:])if route_key(edge[0],edge[1])not in tubes]
				if not self.can_add_tubes(two_hop_tubes,degrees,tubes):continue
				added_two_hop=False;old_score=self.score_from_pods(planned_pods,planned_teleports)[0]
				for candidate_path in two_hop_loop_paths(pad.id,via.id,module.id):
					entry_day=candidate_path.index(module.id);route_options.append((candidate_path,two_hop_tubes,entry_day,len(candidate_path)-1))
					if len(pod_ids)>=MAX_PODS:continue
					upgrade_cost,upgrades=self.path_upgrade_plan(candidate_path,two_hop_tubes,tubes,edge_schedule);two_hop_cost=sum(tube_cost(self.buildings[a],self.buildings[b])for(a,b)in two_hop_tubes)+POD_COST+upgrade_cost
					if two_hop_cost>budget:continue
					score=self.score_added_path(planned_pods,planned_teleports,old_score,candidate_path)
					if score<=0:continue
					_,delivered,services=self.entry_service_bundle(pad,module.id,entry_day,len(candidate_path)-1,module_load,service_counts,planned_adjacency);candidate=Candidate(score,two_hop_cost,pad.id,module.id,astronaut_type,candidate_path,two_hop_tubes,upgrades,delivered=delivered,services=services);candidates.append(candidate);added_two_hop=True
				segment_candidate=self.segment_shuttle_candidate(pad,module,astronaut_type,path,two_hop_tubes,tubes,edge_schedule,planned_pods,planned_teleports,budget,pod_ids)
				if segment_candidate is not None:candidates.append(segment_candidate);added_two_hop=True
				if self.path_has_pod_coverage([pad.id,via.id],planned_pods):
					transfer_path=[via.id,module.id,via.id];transfer_tubes=[]if route_key(via.id,module.id)in tubes else[(via.id,module.id)];route_options.append((transfer_path,transfer_tubes,3,2))
					if len(pod_ids)<MAX_PODS:
						upgrade_cost,upgrades=self.path_upgrade_plan(transfer_path,transfer_tubes,tubes,edge_schedule);transfer_cost=sum(tube_cost(self.buildings[a],self.buildings[b])for(a,b)in transfer_tubes)+POD_COST+upgrade_cost
						if transfer_cost<=budget:delivered=monthly_pod_deliveries(count,3,1,2);score=monthly_score(delivered,3,current_load,period=2)*self.months_left();candidate=Candidate(score,transfer_cost,pad.id,module.id,astronaut_type,transfer_path,transfer_tubes,upgrades,delivered=delivered);candidates.append(candidate);added_two_hop=True
				if added_two_hop:break
				if two_hop_options>=6:break
		if route_options:candidates.extend(self.reroute_candidates(pad,module,astronaut_type,count,current_load,tubes,edge_schedule,rerouted_pod_ids,route_options,budget))
		if(count>=self.teleport_threshold()or not candidates)and pad.id not in teleport_used and module.id not in teleport_used and TELEPORT_COST<=budget:score=monthly_teleport_score(count,current_load)*self.months_left();candidates.append(Candidate(score,TELEPORT_COST,pad.id,module.id,astronaut_type,teleport=(pad.id,module.id),delivered=count))
		return candidates
	def segment_shuttle_candidate(self,pad,module,astronaut_type,path,new_tubes,tubes,edge_schedule,planned_pods,planned_teleports,budget,pod_ids):
		segment_options=[[[path[0],path[1],path[0]]]]
		for(a,b)in zip(path[1:],path[2:]):segment_options.append([[a,b,a],[b,a,b]])
		if len(pod_ids)+len(segment_options)>MAX_PODS:return
		old_score=self.score_from_pods(planned_pods,planned_teleports)[0];best=None
		for selected_paths in product(*segment_options):
			...
			if score<=0:continue
			delivered=service_delivered[pad.id,astronaut_type];services=[(pad.id,astronaut_type,service_paths[pad.id,astronaut_type][-1],delivered)];candidate=Candidate(score,cost,pad.id,module.id,astronaut_type,segment_paths[0],new_tubes,upgrades,delivered=delivered,services=services,extra_paths=segment_paths[1:])
			if best is None or(candidate.score,candidate.delivered,candidate.efficiency)>(best.score,best.delivered,best.efficiency):best=candidate
		return best
	def entry_service_bundle(self,pad,entry_id,entry_day,period,module_load,service_counts,planned_adjacency):
		score=0;delivered_total=0;services=[]
		for(astronaut_type,module_id,distance)in self.reachable_service_entries(pad,entry_id,service_counts,planned_adjacency):first_day=entry_day+distance;count=pad.demand[astronaut_type];delivered=monthly_pod_deliveries(count,first_day,1,max(2,period));score+=monthly_score(delivered,first_day,module_load[module_id],period=max(2,period))*self.months_left();delivered_total+=delivered;services.append((pad.id,astronaut_type,module_id,delivered))
		return score,delivered_total,services
	def reachable_service_entries(self,pad,entry_id,service_counts,adjacency):
		entries={};queue=deque([entry_id]);distances={entry_id:0}
		while queue:
			building_id=queue.popleft();building=self.buildings[building_id]
			if building.kind>0 and building.kind in pad.demand and(pad.id,building.kind)not in service_counts and building.kind not in entries:entries[building.kind]=building.id,distances[building_id]
			for neighbor_id in adjacency.get(building_id,[]):
				if neighbor_id in distances:continue
				distances[neighbor_id]=distances[building_id]+1;queue.append(neighbor_id)
		return[(astronaut_type,module_id,distance)for(astronaut_type,(module_id,distance))in entries.items()]
	def path_has_pod_coverage(self,path,planned_pods):
		edges=set()
		for planned_path in planned_pods.values():edges.update(zip(planned_path,planned_path[1:]))
		return all((a,b)in edges for(a,b)in zip(path,path[1:]))
	def reroute_candidates(self,pad,module,astronaut_type,count,current_load,tubes,edge_schedule,rerouted_pod_ids,route_options,budget):
		candidates=[]
		for old_pod in self.reroutable_pods(rerouted_pod_ids):
			lost_score=self.reroute_loss(old_pod);removed_schedule=self.schedule_without_pod(edge_schedule,old_pod)
			for(path,new_tubes,first_day,period)in route_options:
				upgrade_cost,upgrades=self.path_upgrade_plan(path,new_tubes,tubes,removed_schedule);cost=sum(tube_cost(self.buildings[a],self.buildings[b])for(a,b)in new_tubes)+REROUTE_COST+upgrade_cost
				if cost>budget:continue
				delivered=monthly_pod_deliveries(count,first_day,1,period);score=monthly_score(delivered,first_day,current_load,period=period)*self.months_left()-lost_score;candidate=Candidate(score,cost,pad.id,module.id,astronaut_type,path,new_tubes,upgrades,delivered=delivered,reroute_pod_id=old_pod.id);candidates.append(candidate)
		return candidates
	def reroutable_pods(self,rerouted_pod_ids):pods=[pod for pod in self.pods.values()if pod.id not in rerouted_pod_ids and pod.path];return sorted(pods,key=lambda pod:(self.reroute_loss(pod),pod.id))[:30]
	def reroute_loss(self,pod):
		loss=0;after=self.get_reachable_services(pod.id)
		for((pad_id,astronaut_type),(_,distance))in self.get_reachable_services().items():
			if(pad_id,astronaut_type)not in after:count=self.buildings[pad_id].demand[astronaut_type];delivered=monthly_pod_deliveries(count,distance,1);loss+=monthly_score(delivered,distance,0)*self.months_left()
		return loss
	def schedule_without_pod(self,edge_schedule,pod):
		schedule=edge_schedule.copy()
		for(edge,day)in path_edge_days(pod.path):
			schedule[edge,day]-=1
			if schedule[edge,day]<=0:del schedule[edge,day]
		return schedule
	def shortest_tube_path(self,start_id,finish_id,tubes):
		if start_id==finish_id:return[start_id]
		graph={}
		for(a,b)in tubes:graph.setdefault(a,[]).append(b);graph.setdefault(b,[]).append(a)
		queue=deque([start_id]);parent={start_id:start_id}
		while queue:
			building_id=queue.popleft()
			for neighbor_id in graph.get(building_id,[]):
				if neighbor_id in parent:continue
				parent[neighbor_id]=building_id
				if neighbor_id==finish_id:return unwind_path(parent,start_id,finish_id)
	...
	def path_upgrade_plan(self,path,new_tubes,tubes,edge_schedule):
		...
		for(edge,required_capacity)in required_capacities.items():
			...
			for new_capacity in range(capacity+1,required_capacity+1):a,b=edge;cost+=tube_cost(self.buildings[a],self.buildings[b])*new_capacity;upgrades.append(edge)
		return cost,upgrades
	def can_add_tubes(self,new_tubes,degrees,tubes):
		extra=[];extra_keys=set();extra_degrees=Counter()
		for(a,b)in new_tubes:
			key=route_key(a,b)
			if key in tubes or key in extra_keys:continue
			if degrees[a]+extra_degrees[a]>=MAX_TUBES_PER_BUILDING or degrees[b]+extra_degrees[b]>=MAX_TUBES_PER_BUILDING:return False
			if not self.can_build_tube(a,b,tubes,extra):return False
			extra.append((a,b));extra_keys.add(key);extra_degrees[a]+=1;extra_degrees[b]+=1
		return True
	def can_build_tube(self,a,b,tubes,extra_tubes):
		if a==b or route_key(a,b)in tubes:return False
		first=self.buildings[a];second=self.buildings[b]
		for building in self.buildings.values():
			if building.id not in(a,b)and point_on_segment(building,first,second):return False
	...
	def best_capacity_candidate(self,serviced,tubes,direct_counts,edge_schedule,planned_pods,planned_teleports,teleported_pairs,budget,pod_ids):
		...
		for pad in self.get_landing_pads():
			for(astronaut_type,count)in pad.demand.items():
				if(pad.id,astronaut_type)not in serviced or(pad.id,astronaut_type)in teleported_pairs:continue
				...
				for module in modules:
					edge=route_key(pad.id,module.id);pods_on_edge=direct_counts[pad.id,module.id]
					if pods_on_edge<=0 or pods_on_edge>=4:continue
					path=[pad.id,module.id,pad.id];upgrade_cost,upgrades=self.path_upgrade_plan(path,[],tubes,edge_schedule);cost=POD_COST+upgrade_cost;score_gain=self.score_added_path(planned_pods,planned_teleports,old_score,path)
					if cost<=budget and score_gain>0:
						...
						if best is None or(candidate.score,candidate.efficiency)>(best.score,best.efficiency):best=candidate
		return best
	def get_service_paths(self):return self.service_paths_from_adjacency(self.get_pod_adjacency())
	def service_paths_from_adjacency(self,adjacency):
		paths={}
		for pad in self.get_landing_pads():
			queue=deque([pad.id]);parent={pad.id:pad.id}
			while queue:
				building_id=queue.popleft();building=self.buildings[building_id]
				if building.kind>0 and building.kind in pad.demand and(pad.id,building.kind)not in paths:paths[pad.id,building.kind]=unwind_path(parent,pad.id,building_id)
				for neighbor_id in adjacency.get(building_id,[]):
					if neighbor_id in parent:continue
					parent[neighbor_id]=building_id;queue.append(neighbor_id)
		return paths
	def best_baseline_direct_candidate(self,serviced,tubes,dedicated_edge_counts,edge_schedule,planned_pods,planned_teleports,teleported_pairs,budget,pod_ids):
		if len(pod_ids)>=MAX_PODS or budget<POD_COST:return
		best=None;old_score=self.score_from_pods(planned_pods,planned_teleports)[0]
		for pad in self.get_landing_pads():
			for(astronaut_type,count)in pad.demand.items():
				if(pad.id,astronaut_type)not in serviced or(pad.id,astronaut_type)in teleported_pairs:continue
				for module in self.get_modules_by_type()[astronaut_type]:
					edge=route_key(pad.id,module.id)
					if edge not in tubes or dedicated_edge_counts[edge]:continue
					path=[pad.id,module.id,pad.id];upgrade_cost,upgrades=self.path_upgrade_plan(path,[],tubes,edge_schedule);score=self.score_added_path(planned_pods,planned_teleports,old_score,path)
					if POD_COST+upgrade_cost>budget or score<=0:continue
					candidate=Candidate(score,POD_COST+upgrade_cost,pad.id,module.id,astronaut_type,path,upgrades=upgrades,delivered=count)
					if best is None or candidate.efficiency>best.efficiency or candidate.efficiency==best.efficiency and candidate.score>best.score:best=candidate
		return best
	...
	def best_baseline_path_candidate(self,tubes,dedicated_edge_counts,edge_schedule,planned_pods,planned_teleports,teleported_pairs,budget,pod_ids):
		...
		for((pad_id,astronaut_type),path)in self.get_service_paths().items():
			if(pad_id,astronaut_type)in teleported_pairs:continue
			for(a,b)in zip(path,path[1:]):
				edge=route_key(a,b)
				if edge not in tubes or dedicated_edge_counts[edge]:continue
				count=self.buildings[pad_id].demand[astronaut_type];candidate_path=[a,b,a];upgrade_cost,upgrades=self.path_upgrade_plan(candidate_path,[],tubes,edge_schedule);score=self.score_added_path(planned_pods,planned_teleports,old_score,candidate_path)
				if POD_COST+upgrade_cost>budget or score<=0:continue
				...
				if best is None or candidate.efficiency>best.efficiency or candidate.efficiency==best.efficiency and candidate.score>best.score:best=candidate
		return best
	def best_speed_candidate(self,serviced,service_counts,tubes,direct_counts,edge_schedule,dedicated_edge_counts,planned_pods,planned_teleports,rerouted_pod_ids,teleport_used,teleported_pairs,budget,pod_ids):
		candidates=[];baseline_candidate=self.best_baseline_direct_candidate(serviced,tubes,dedicated_edge_counts,edge_schedule,planned_pods,planned_teleports,teleported_pairs,budget,pod_ids)
		if baseline_candidate is not None:candidates.append(("baseline_pod",baseline_candidate))
		path_candidate=self.best_baseline_path_candidate(tubes,dedicated_edge_counts,edge_schedule,planned_pods,planned_teleports,teleported_pairs,budget,pod_ids)
		if path_candidate is not None:candidates.append(("baseline_pod",path_candidate))
		if candidates:return max(candidates,key=lambda item:(item[1].score,item[1].efficiency))
		capacity_candidate=self.best_capacity_candidate(serviced,tubes,direct_counts,edge_schedule,planned_pods,planned_teleports,teleported_pairs,budget,pod_ids)
		if capacity_candidate is not None:candidates.append(("capacity",capacity_candidate))
		teleport_candidate=self.best_teleport_speed_candidate(planned_pods,planned_teleports,teleport_used,budget)
		if teleport_candidate is not None:candidates.append(("speed_teleport",teleport_candidate))
		if not candidates:return
		if self.months_left()<=2:return max(candidates,key=lambda item:(item[1].score,item[1].efficiency))
		return max(candidates,key=lambda item:(item[1].efficiency,item[1].score))
	def best_teleport_speed_candidate(self,planned_pods,planned_teleports,teleport_used,budget):
		if budget<TELEPORT_COST:return
		best=None;old_score,_,_,_,_,service_paths=self.score_from_pods(planned_pods,planned_teleports)
		for((pad_id,astronaut_type),_)in service_paths.items():
			if pad_id in teleport_used:continue
			for module in self.get_modules_by_type()[astronaut_type]:
				if module.id in teleport_used:continue
				new_teleports=dict(planned_teleports);new_teleports[pad_id]=module.id;new_score=self.score_from_pods(planned_pods,new_teleports)[0];candidate=Candidate((new_score-old_score)*self.months_left(),TELEPORT_COST,pad_id,module.id,astronaut_type,teleport=(pad_id,module.id),delivered=self.buildings[pad_id].demand[astronaut_type])
				if candidate.score>0 and(best is None or candidate.score>best.score):best=candidate
		return best
	def apply_candidate(self,reason,candidate,actions,serviced,service_counts,module_load,degrees,teleport_used,planned_teleports,tubes,direct_pod_counts,edge_schedule,dedicated_edge_counts,planned_pods,budget,pod_ids):
		if candidate.reroute_pod_id is not None:
			...
			for(pad_id,astronaut_type,module_id,_)in self.pod_services(old_pod):
				...
				if service_counts[pad_id,astronaut_type]<=0:del service_counts[pad_id,astronaut_type];serviced.discard((pad_id,astronaut_type));module_load[module_id]-=self.buildings[pad_id].demand[astronaut_type]
		for(a,b)in candidate.tubes:
			if route_key(a,b)in tubes:continue
			actions.append(f"TUBE {a} {b}");tubes[route_key(a,b)]=1;degrees[a]+=1;degrees[b]+=1
		for(a,b)in candidate.upgrades:actions.append(f"UPGRADE {a} {b}");tubes[route_key(a,b)]+=1
		...
		if reason=="service":
			...
			if candidate.services:services=candidate.services
			elif reason in("baseline_pod","capacity"):services=[(candidate.pad_id,candidate.astronaut_type,candidate.module_id,candidate.delivered)]
			else:services=[]
			for(pad_id,astronaut_type,module_id,delivered)in services:
				was_serviced=(pad_id,astronaut_type)in serviced;serviced.add((pad_id,astronaut_type));service_counts[pad_id,astronaut_type]+=1
				if not was_serviced:module_load[module_id]+=delivered
		return budget-candidate.cost
	def months_left(self):return MAX_MONTHS-self.month
	def min_efficiency(self):return .55 if self.month<14 else .8
	def teleport_threshold(self):
		if self.month<=3:return 55
		if self.month<=10:return 70
		return 90
def format_debug_node(building):
	if building.kind==0:grouped=[astronaut_type for astronaut_type in sorted(building.demand)for _ in range(building.demand[astronaut_type])];demand=",".join(f"{astronaut_type}:{building.demand[astronaut_type]}"for astronaut_type in sorted(building.demand))if building.order==grouped else",".join(map(str,building.order));demand=demand or"none";return f"landing {building.id} {building.x} {building.y} {demand}"
	return f"module {building.id} {building.kind} {building.x} {building.y}"
def unique_new_tubes(path,tubes):
	new_tubes=[];seen=set()
	for(a,b)in zip(path,path[1:]):
		key=route_key(a,b)
		if key not in tubes and key not in seen:new_tubes.append((a,b));seen.add(key)
	return new_tubes
def route_key(a,b):return(a,b)if a<b else(b,a)
def tube_cost(a,b):return isqrt(100*((a.x-b.x)*(a.x-b.x)+(a.y-b.y)*(a.y-b.y)))
def point_on_segment(point,a,b):return orientation(a,b,point)==0 and min(a.x,b.x)<=point.x<=max(a.x,b.x)and min(a.y,b.y)<=point.y<=max(a.y,b.y)
def segments_intersect(a,b,c,d):
	o1=orientation(a,b,c);o2=orientation(a,b,d);o3=orientation(c,d,a);o4=orientation(c,d,b)
	if o1==0 and point_on_segment(c,a,b)or o2==0 and point_on_segment(d,a,b):return True
	if o3==0 and point_on_segment(a,c,d)or o4==0 and point_on_segment(b,c,d):return True
...
def monthly_pod_deliveries(count,distance,pod_count,period=None):
	delivered=0;day=distance;period=2*distance if period is None else period
	while day<=MONTH_DAYS and delivered<count:delivered+=min(count-delivered,10*pod_count);day+=period
	return delivered
def monthly_score(count,distance,current_load,pod_count=1,period=None):
	score=0;delivered=0;day=distance;period=2*distance if period is None else period
	while day<=MONTH_DAYS and delivered<count:
		batch=min(count-delivered,10*pod_count)
		for _ in range(batch):score+=max(0,50-day)+max(0,50-current_load-delivered);delivered+=1
		day+=period
	return score
def monthly_teleport_score(count,current_load):return sum(50+max(0,50-current_load-passenger_ind)for passenger_ind in range(count))
def next_pod_id(used_ids):
	for pod_id in range(1,MAX_PODS+1):
		if pod_id not in used_ids:return pod_id
	raise RuntimeError("No pod identifiers remain")
if __name__=="__main__":Planner().play()

...
3. Stage 1. Single pod approximation.
   1. Each pod makes an individual list of assignments, assuming no other pods exist.
      1. Assignments on the list are sorted as follows.
         ...
         For each passenger in that batch calculate the number of speed and diversity points that would be given by delivering that passenger to its
         destination. Divide those numbers by the number of edges in its delivery path + the number of edges to reach the origin.
         That tuple is passenger's efficiency. Add up passenger efficiencies to get total batch efficiency (for a given pod).

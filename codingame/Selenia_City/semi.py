...
class Bundle:
    ...
    def fingerprint(self) -> tuple:
        return self.tubes, self.teleport, tuple(spec.pod_id for spec in self.pod_specs), self.upgrades
...
class PlanState:
    ...
    planned_pod_paths: dict[int, set[Pair]] = field(default_factory=dict)
...
class SimulationResult:
    ...
    assignment_table: list[str] = field(default_factory=list)
...
class Planner:
    ...
    def choose_actions(self) -> list[str]:
        ...
        print("\nAssignments:\n" + "\n".join(final_result.assignment_table), file=sys.stderr)
    ...
    def projection_bundle(self, owner: PoolOwner, template: Bundle, before: PlanState, after: PlanState, label: str) -> Bundle:
        ...
        specs = [PodSpec(pod_id) for pod_id in sorted(set(before.pods) & set(after.pods))
            if after.pods[pod_id].dynamic and not before.pods[pod_id].dynamic]
        specs.extend(PodSpec(0) for _ in set(after.pods) - set(before.pods))
    ...
    def replay_bundle_on_state(self, state: PlanState, bundle: Bundle) -> PlanState:
        ...
        copied = PlanState(dict(state.tubes), dict(state.teleports), pods, list(state.actions), list(state.placeholders),
            set(state.planned_pods), {pod_id: set(edges) for pod_id, edges in state.planned_pod_paths.items()},
    ...
    def replay_bundle_sequence(self, selected: list[Bundle]) -> PlanState:
        pods = {pod_id: PodPlan(pod_id, pod.path[:]) for pod_id, pod in self.pods.items()}
    ...
    def remove_planned_pod(self, state: PlanState, pod_id: int):
        if pod_id in self.pods:
            ...
            state.pods[pod_id] = PodPlan(pod_id, self.pods[pod_id].path[:])
        else:
            state.cost -= POD_COST
            del state.pods[pod_id]
        state.planned_pods.remove(pod_id)
        del state.planned_pod_paths[pod_id]
    ...
    def apply_bundle(self, state: PlanState, bundle: Bundle):
        ...
        for spec in bundle.pod_specs:
            if spec.pod_id:
                ...
                state.placeholders.append((len(state.actions), pod_id))
                state.actions.append("")
            state.planned_pod_paths[pod_id] = set(bundle.path_edges)
    ...
    def score_state(self, state: PlanState, keep_dynamic_paths: bool = False) -> SimulationResult:
        ...
        if keep_dynamic_paths:
            ...
            fixed_result.assignment_table = dynamic_result.assignment_table
        return fixed_result
    ...
    def copy_simulation_result(self, result: SimulationResult) -> SimulationResult:
        ...
        copy.assignment_table = result.assignment_table[:]
        return copy
    ...
    def fixed_dynamic_state(self, state: PlanState, dynamic_paths: dict[int, list[int]]) -> PlanState:
        ...
        return PlanState(dict(state.tubes), dict(state.teleports), pods, list(state.actions), list(state.placeholders), set(state.planned_pods),
            {pod_id: set(edges) for pod_id, edges in state.planned_pod_paths.items()}, dict(state.planned_pod_pools), set(state.planned_tubes), state.cost)
    ...
    def simulate(self, state: PlanState, keep_dynamic_paths: bool = False) -> SimulationResult:
        ...
        fixed_schedule = self.fixed_assignment_schedule(state, distances, wanted_edges, path_demands, fixed_pods) if dynamic_pods else []
        ...
        for day in range(MONTH_DAYS):
            ...
            assignments, preferences, assignment_current, priorities = self.dispatch_dynamic_paths(day, active, dynamic_pods, dynamic_pending,
                dynamic_current, fixed_assignments, fixed_reservations, reserved_loads, result, state, graph, components, queues, wanted_edges,
                demand)
            ...
            assignments, path_orders, requests, moves = self.resolve_dispatch_congestion(assignments, preferences, requests, moves,
                fixed_pods, dynamic_pods, pod_positions, dynamic_current, assignment_current, dynamic_pending, graph, queues, wanted_edges,
                result, state, demand, fixed_assignments, fixed_reservations, priorities)
            if keep_dynamic_paths:
                ...
                fixed_text = ", ".join(f"P{pod_id}=" + "/".join("-".join(map(str, path.nodes)) for path in paths)
                    for pod_id, paths in fixed_assignments.items()) or "-"
                assigned_text = ", ".join("P{}={}".format(pod_id, "-".join(map(str, path.nodes)))
                    for pod_id, path in sorted(assignments.items())) or "-"
                result.assignment_table.append(f"day {day + 1}: loads={loads}; fixed={fixed_text}; assigned={assigned_text}")
    ...
    def fixed_assignment_schedule(self, state: PlanState, distances: dict[int, dict[int, int]],
            ...
            fixed_pods: list[tuple[int, PodPlan]]) -> list[tuple[dict[int, set[PathDemand]], Counter[LoadKey]]]:
        """Builds fixed_pods daily load reservations from state, distances, wanted_edges, and path_demands."""
        if not fixed_pods:
            return [({}, Counter()) for _ in range(MONTH_DAYS)]
        queues = self.initial_queues(distances)
        result = SimulationResult()
    ...
    def fixed_load_assignments(self, fixed_pods: list[tuple[int, PodPlan]], pod_positions: dict[int, int],
        ...
        """Infers fixed_pods assignments and reservations from pod_positions, active loads, queues, wanted_edges, and result."""
        moves = {}
        for pod_id, pod in fixed_pods:
    ...
    def active_path_demands(self, path: PathDemand, queues: dict[int, list[Passenger]],
        ...
        """Builds and returns active suffixes for path from queues and wanted_edges, capped by result delivery counts."""
    ...
    def dispatch_dynamic_paths(self, day: int, active: list[PathDemand], dynamic_pods: list[tuple[int, PodPlan]],
            ...
            fixed_reservations: Counter[tuple[Pool, int]], reserved_loads: Counter[LoadKey], result: SimulationResult, state: PlanState,
        ...
        """Builds dynamic_pods assignments from day, active loads, pod state, reservations, topology, queues, and demand."""
        ...
        initial_capacity = {path: self.path_capacity_excess(path, {}, fixed_assignments, state) for path in dispatchable}
        preferences = {}
        for pod_id in pod_ids:
        ...
        if day == 0:
            ...
            for pod_id in pod_ids:
                options = [path for path in preferences[pod_id] if not counts[path] and components[path.nodes[0]] in uncovered
                    ...
                    and not self.dispatch_supply_exceeded(assignments | {pod_id: path}, assignment_current, fixed_reservations, queues, result)]
                if options:
        ...
        while remaining:
            capacity_blocked = {path for path in dispatchable
                if self.path_capacity_excess(path, assignments, fixed_assignments, state)}
            proposals = {}
            ...
            for path, pod_options in proposals.items():
                ...
                pod_id = min(pod_options, key=lambda item: (pending[item] != first_edge[::-1],
                    ...
                    graph_distance(graph, assignment_current[item], path.nodes[0]), item))
                if self.path_capacity_excess(path, assignments, fixed_assignments, state) or \
                        self.dispatch_supply_exceeded(assignments | {pod_id: path}, assignment_current, fixed_reservations, queues, result):
                    unavailable.add(path)
                    continue
                assignments[pod_id] = path
                counts[path] += 1
                remaining.remove(pod_id)
        return assignments, preferences, assignment_current, priorities
    ...
    def dispatch_supply_exceeded(self, assignments: dict[int, PathDemand], current: dict[int, int],
            fixed_reservations: Counter[tuple[Pool, int]], queues: dict[int, list[Passenger]], result: SimulationResult) -> bool:
        """Returns whether dynamic assignments and fixed_reservations exceed matching queues; current and result limit reservations."""
    ...
    def resolve_dispatch_congestion(self, assignments: dict[int, PathDemand], preferences: dict[int, list[PathDemand]],
            ...
            assignment_current: dict[int, int], pending: dict[int, DirectedPair], graph: dict[int, list[int]], queues: dict[int, list[Passenger]],
            wanted_edges: dict[tuple[int, int], tuple[DirectedPair, ...]], result: SimulationResult, state: PlanState,
            demand: Counter[DirectedPair], fixed_assignments: dict[int, set[PathDemand]],
            fixed_reservations: Counter[tuple[Pool, int]], priorities: dict[PathDemand, int]) -> tuple:
        """Reassigns assignments using preferences and state, returning assignments, orders, requests, and moves."""
        ...
        while blocked:
            ...
            for pod_id, path, other_id in trials:
                ...
                if self.dispatch_supply_exceeded(trial, assignment_current, fixed_reservations, queues, result):
                    continue
                other_assignments = dict(trial)
                del other_assignments[pod_id]
                if self.path_capacity_excess(path, other_assignments, fixed_assignments, state):
                    continue
                if other_id >= 0:
                    del other_assignments[other_id]
                    if self.path_capacity_excess(previous, other_assignments | {pod_id: path}, fixed_assignments, state):
                        continue
                counts = Counter(trial.values())
                if any(counts[a] > counts[b] + 1 and not self.path_capacity_excess(b, trial, fixed_assignments, state)
                        for a in loads for b in loads if priorities[a] == priorities[b]) or \
                        any(len(a.nodes) < len(b.nodes) and counts[a] > counts[b] and
                            not self.path_capacity_excess(b, trial, fixed_assignments, state)
                            for a in loads for b in loads if priorities[a] == priorities[b]) or \
                        any(priorities[a] < priorities[b] and counts[b] and
                            not self.path_capacity_excess(a, trial, fixed_assignments, state) for a in loads for b in loads):
                    continue
                orders = self.path_orders_for_assignments(trial, assignment_current, graph)
        ...
        return assignments, self.path_orders_for_assignments(assignments, assignment_current, graph), requests, moves
    ...
    def path_capacity_excess(self, candidate: PathDemand, assignments: dict[int, PathDemand],
            fixed_assignments: dict[int, set[PathDemand]], state: PlanState) -> bool:
        def projected() -> Counter[Pair]:
            loads = Counter(fixed_loads)
            ...
            return loads
        workers = Counter(path.nodes for path in assignments.values())
        fixed_loads = Counter()
        for paths in fixed_assignments.values():
            fixed_loads.update({route_key(a, b) for path in paths for a, b in zip(path.nodes, path.nodes[1:])})
        before = projected()
    ...
    def path_pod_requests(self, fixed_pods: list[tuple[int, PodPlan]], dynamic_pods: list[tuple[int, PodPlan]],
        ...
        for pod_id, _ in dynamic_pods:
            if pending[pod_id] != (-1, -1):
                requests[pod_id] = pending[pod_id]
                continue
            if pod_id not in assignments:
                continue
            demand = assignments[pod_id]
            path = demand.nodes
            order = path_orders[demand]
            edge_count = len(path) - 1
            index = order.index(pod_id)
            if len(order) <= edge_count:
                start = index * edge_count // len(order)
                finish = (index + 1) * edge_count // len(order)
            else:
                start = min(edge_count - 1, index * edge_count // len(order))
                finish = start + 1
            ...
            if current[pod_id] == -1:
                ...
                continue
            if current[pod_id] not in segment:

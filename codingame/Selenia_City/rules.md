...
4. Reassign pods as necessary to conform to the capacity and uniformity constraints.
   ...
   2. For each conflicting group keep the maximum number of pods from the group supported by capacity and uniformity assigned to their original preference.
      1. Prefer to keep pods whose extra distance to next eligible assignment is larger.
      2. On tie, prefer to keep pods that are closer to load's origin.
      3. On tie, prefer to keep pods with smaller id.
      4. Resolve loads in the ascending order of the number of capacity slots.
...
6. Reassign pods as necessary to fix some of the edge conflicts. Specifically:
   ...
   3. A lower id pod assigned to a low priority load that has an edge conflict with a higher id pod assigned to a normal priority load should prefer moving
...
8. If an assigned pod was not able to move along its ideal path for any reason, increase congestion counter on the bottleneck edge of that path.
   1. Ideal path is defined as the path that would have been assigned if all edges had infinite capacity.
   I.e. the path assigned after passenger-capacity and uniformity resolution stages,
   but before anything that takes into account finite edge capacity (e.g. path-capacity and edge conflict resolution stages).
      1. Any alternative paths of the same length as originally assigned ideal path are considered equivalent to the original one.
      Being able to take any ideal path does not increase congestion counter on the original ideal path.
   2. Bottleneck edge is defined as the first edge of the ideal path by default.
      1. Exception: If the pod was not able to take its ideal path due to path capacity, bottleneck edge is defined as the closest to origin edge
      with the smallest capacity along the delivery way.

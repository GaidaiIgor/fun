...
class Candidate:
    ...
    def efficiency(self) -> float:
        return self.marginal_gain / self.marginal_cost if self.marginal_cost > 0 else inf
class Planner:
    ...
    def next_candidate(self, owner: PoolOwner, pair: PoolOwner, group: Pool, current_state: PlanState,
        ...
        for option, action_text in plans:
            ...
            marginal_gain = result.score - current_result.score
            marginal_cost = state.cost - current_state.cost
            checkpoint = f"({marginal_gain:+d})"
            efficiency = marginal_gain / marginal_cost if marginal_cost > 0 else inf if marginal_gain > 0 else 0
            ...
            if global_gain > 0 and result.score > current_result.score:
                candidate = Candidate(bundle, pair, marginal_gain, marginal_cost, option.layouts, option.layout_key, state)
    ...
    def throughput_options(self, owner: PoolOwner, group: Pool, parent: PlanOption, parent_efficiency: float,
        ...
        while parent.state.cost <= self.resources:
            ...
            pod_metrics = self.option_metrics(pod_option, checkpoint_score, checkpoint_cost)
    ...
    def fixed_assignment_schedule(self, state: PlanState, distances: dict[int, dict[int, int]],
        ...
        for day in range(MONTH_DAYS):
            ...
            for passengers in queues.values():
                passengers.sort(key=BY_ID)
            active, _ = self.daily_loads(state, queues, module_distances, result, set())
    ...
    def daily_loads(self, state: PlanState, queues: dict[int, list[Passenger]], module_distances: dict[int, dict[int, int]],
        ...
        for (pool, node_id), count in sorted(counts.items()):
            ...
            options = tuple(sorted(module_id for module_id in modules if module_distances[module_id][node_id] == distance))
    ...
    def dispatch_dynamic_paths(self, active: list[PathDemand], dynamic_pods: list[tuple[int, PodPlan]], current: dict[int, int],
        ...
        for pod_id, _ in dynamic_pods:
            ...
            batches = {path: self.boarding_batch(path.nodes[:2], queues, wanted_edges, reserved) for path in options}
            evaluated = []
            for path in options:
                priority = path.priority
                if priority > 0:
                    levels = Counter(passenger_priorities.get(((passenger.pad_id, passenger.kind), *path.nodes[:2]), 0)
                        for passenger in batches[path])

...
def project_progress_bonus(exp, final_exp, projects, opp_exp=None):
    bonus = 0.0
    opp_exp = opp_exp or [0] * 5

    for pr in projects:
        before = project_deficit(exp, pr)
        after = project_deficit(final_exp, pr)
        opp_gap = sum(project_deficit(opp_exp, pr))
        ...
        bonus += 2.8 * max(0, max(before) - max(after))

        if opp_gap <= 2 and before_sum <= 3:
            bonus += 18.0 * max(0, before_sum - after_sum)
            if after_sum == 0 and before_sum > 0:
                bonus += 35.0

    return bonus


def gain_bonus(sample, exp, projects, opp_exp=None):
    gi = mi(sample.gain)
    if gi < 0:
        return 0.0
    ...
    for pr in projects:
        my_gap = sum(project_deficit(exp, pr))
        opp_gap = sum(project_deficit(opp_exp, pr))
...
def shortage_penalty(req, storage, avail, remaining, require_available):
    need = to_collect_for_req(req, storage)
    ...
    for i in range(5):
        if need[i] > 0 and avail[i] <= 1:
            p += 2.5 * need[i]
        if req[i] >= 7 and storage[i] + avail[i] < req[i]:
            p += 35.0 if remaining < 90 else 12.0

    return p


def plan_value(order, req, final_exp, storage, avail, exp, projects, remaining, target, require_available, opp_exp):
    health = sum(s.health for s in order)
    need = to_collect_for_req(req, storage)
    est = estimate_finish_turns(order, need, target)

    value = health
    value += sum(gain_bonus(s, exp, projects, opp_exp) for s in order)
    value += project_progress_bonus(exp, final_exp, projects, opp_exp)
    value += 1.8 * (len(order) - 1)
    value -= shortage_penalty(req, storage, avail, remaining, require_available)

    if est + 1 > remaining:
        value -= 150.0 + 8.0 * (est + 1 - remaining)
...
def select_best_plan(samples, exp, storage, avail, projects, remaining, target, require_available, opp_exp=None, ignore_dropped=True):
    diagnosed = [s for s in samples if s.diagnosed and (not ignore_dropped or s.id not in dropped_ids)]
    best = None
    ...
    for r in range(1, len(diagnosed) + 1):
        for subset in combinations(diagnosed, r):
            for order in permutations(subset):
                ...
                val = plan_value(
                    order, req, final_exp, storage, avail, exp, projects,
                    remaining, target, require_available, opp_exp or [0] * 5
                )
...
def best_single_value(s, exp, storage, avail, projects, remaining, target, opp_exp):
    p = select_best_plan([s], exp, storage, avail, projects, remaining, target, False, opp_exp)
...
def choose_rank(exp, projects, remaining):
    ...
    if remaining > 70 and min(exp) >= 1 and et >= 12:
        return 3

    return 2


def choose_completion_sample(makeable, exp, storage, projects, opp_exp):
    best = None
    ...
    for s in makeable:
        ...
        score = s.health
        score += gain_bonus(s, exp, projects, opp_exp)
        score += project_progress_bonus(exp, e2, projects, opp_exp)
...
def choose_cloud_sample(cloud, carried, exp, storage, avail, projects, remaining, target, opp_exp):
    if len(carried) >= 3:
        return None

    current = select_best_plan(carried, exp, storage, avail, projects, remaining, target, True, opp_exp)
    ...
    for s in cloud:
        ...
        combined = select_best_plan(carried + [s], exp, storage, avail, projects, remaining, target, True, opp_exp)
        if combined is None:
            continue

        single_val = best_single_value(s, exp, storage, avail, projects, remaining, target, opp_exp)
        score = single_val + 0.8 * (combined["value"] - current_val)
...
def should_escape_to_diag(loose_plan, storage, avail, remaining):
    if loose_plan is None:
        return True

    z = zero_block_types(loose_plan["req"], storage, avail)
    shortage = shortage_for_req(loose_plan["req"], storage, avail)

    if z and remaining < 130:
        return True

    if sum(shortage) >= 3 and storage_total(storage) >= 5:
        return True

    if remaining < 60 and sum(shortage) > 0:
        return True

    return False


def choose_drop_sample(carried, exp, storage, avail, projects, remaining, target, loose_plan, opp_exp):
    diagnosed = [s for s in carried if s.diagnosed]
    if not diagnosed:
        return None

    strict = select_best_plan(diagnosed, exp, storage, avail, projects, remaining, target, True, opp_exp)
    if strict is not None:
        return None

    # v5 restraint: don't donate samples just because one molecule is scarce.
    if not should_escape_to_diag(loose_plan, storage, avail, remaining):
        return None

    candidates = []
    plan_ids = set(loose_plan["ids"]) if loose_plan else set()
    blocked_types = zero_block_types(loose_plan["req"], storage, avail) if loose_plan else []

    for s in diagnosed:
        ec = effective_cost(s, exp)
        val = best_single_value(s, exp, storage, avail, projects, remaining, target, opp_exp)
        badness = -val

        if sum(ec) > 10:
            badness += 100

        if s.id in plan_ids and blocked_types:
            for i in blocked_types:
                if ec[i] > 0:
                    badness += 80 + 15 * ec[i]
...
def summarize_samples(samples):
    ...
    for s in samples:
        if s.diagnosed:
            out.append("{}:r{}:{}:{}:{}".format(s.id, s.rank, s.health, s.gain, "".join(map(str, s.cost))))
        else:
...
def decide(projects, robots, avail, samples):
    ...
    strict_plan = select_best_plan(
        diagnosed, me.expertise, me.storage, avail, projects,
        remaining, me.target, True, opp.expertise
    )

    loose_plan = strict_plan or select_best_plan(
        diagnosed, me.expertise, me.storage, avail, projects,
        remaining, me.target, False, opp.expertise
    )
    ...
    if me.target == LAB:
        ...
        if makeable:
            s = choose_completion_sample(makeable, me.expertise, me.storage, projects, opp.expertise)
    ...
    if me.target == MOLMOD:
        if makeable:
            ...
            if mol is not None and strict_plan is not None and strict_plan["est"] + 2 < remaining and remaining > 35:
                clear_block_memory()
        ...
        if loose_plan is not None:
            ...
            if mol is not None and (count <= 2 or not zero_block_types(loose_plan["req"], me.storage, avail)):
                return "CONNECT {}".format(mol), loose_plan, "mol_collect_loose"

            if zero_block_types(loose_plan["req"], me.storage, avail) and count <= 1 and remaining > 80:
    ...
    if me.target == DIAG:
        ...
        if loose_plan is not None:
            mol = choose_molecule(loose_plan, me.storage, avail, me.expertise)
            if mol is not None and not should_escape_to_diag(loose_plan, me.storage, avail, remaining):
                clear_block_memory()
                return goto(MOLMOD, me), loose_plan, "diag_to_mol_loose"

            bad = choose_drop_sample(
                carried, me.expertise, me.storage, avail, projects,
                remaining, me.target, loose_plan, opp.expertise
            )
            if bad is not None:
        ...
        if len(carried) < 3:
            s = choose_cloud_sample(
                ...
                remaining, me.target, opp.expertise
            )
            if s is not None:
                clear_block_memory()
...
while True:
    ...
    if plan is None:
        ...
        plan_s = "ids={} req={} need={} val={:.1f} est={}".format(
            plan["ids"], plan["req"], to_collect_for_req(plan["req"], me.storage),
            plan["value"], plan["est"]
        )

    print(
        "T{} {} eta={} score={} opp={} store={} exp={} oppExp={} avail={} carried={} plan={} dropped={} block={}x{} reason={} -> {}".format(
            turn, me.target, me.eta, me.score, opp.score, me.storage,
            me.expertise, opp.expertise, available, summarize_samples(carried),
            plan_s, sorted(dropped_ids), blocked_key, blocked_count, reason, action
        ),
        file=sys.stderr,
    )

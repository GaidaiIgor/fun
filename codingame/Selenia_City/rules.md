...
4. Reassign pods as necessary to conform to the capacity and uniformity constraints.
   ...
   2. For each conflicting group keep the maximum number of pods from the group supported by capacity and uniformity assigned to their original preference.
      1. Prefer to keep pods with larger extra distances to their next alternative load that conforms to capacity and uniformity.
      2. On tie, prefer to keep pods that are closer to load's origin.
      3. On tie, prefer to keep pods with smaller id.
   3. The remaining pods are reassigned to other loads allowed by load capacity and uniformity constraints in the order of their preference lists.
      1. Being reassigned due to path capacity increases congestion counter on the closest to origin edge with the smallest capacity along the delivery way.
...
5. Reassign pods as necessary to fix some of the edge conflicts. Specifically:
   1. If two conflicting pods are scheduled to move through an edge in opposite directions in excess of its capacity
   and swapping their assignments resolves the conflict, then swap them.
   2. If an assigned pod could not move through an edge due to capacity, increase congestion counter on that edge.
